# DIB客户端（Universal Tray Tool, UTT）
## 产品需求文档（PRD）
...
### 1.1 产品定位
DIB客户端（Universal Tray Tool, UTT）是一款基于 Avalonia UI + Prism 的跨平台桌面托盘应用框架。
...
#### 2.2.1 MVP 目标
- 约定插件源码目录：`plugins-src/`
- 约定本地产包中转目录：`plugins/`，该目录不提交到 Git
...
#### 2.3.1 目标定位
- 建立统一的资源中心，用于集中管理插件运行时依赖。
- 资源不仅包括数据库连接，还包括 `Supabase`、`OCR`、人脸识别等后端服务连接。
- 资源中心同时承担“资源注册中心 + 授权中心”职责。

#### 2.3.2 核心流程
- 医疗机构可在资源中心申请注册资源。
- 管理员审批通过后，资源进入可发现状态。
- 托盘根据当前 `使用单位 + 站点 + 插件` 发现可申请资源。
- 站点发起资源使用申请，审批通过后形成授权绑定。
- 宿主向插件下发已授权资源，插件受控直连外部资源。

#### 2.3.3 管理原则
- “单位”表示业务主体，“站点”表示部署实例，“插件”表示能力载体。
- 资源授权默认优先采用“站点 + 插件”最小粒度。
- 插件不得长期自行维护连接来源，资源真相由后台统一管理。
- 详细设计见：
  - `docs/plans/2026-04-14-resource-center-design.md`
  - `docs/plans/2026-04-14-resource-center-contracts.md`
  - `docs/plans/2026-04-14-resource-center-api.md`
...
├── Views/
├── Assets/
├── plugins-src/           # 插件源码目录（规划）
├── plugins/               # 本地产包中转目录，生成后忽略
└── appsettings.json
```
...
| v1.2.0 | 插件加载生命周期 + 资源中心一期 | 2026 Q3 |
...
- 插件隔离与安全策略实现复杂度高。
- 需求扩展速度可能超过架构演进速度。
- 资源中心若过早绑定具体第三方配置平台，可能导致业务模型被基础设施产品反向约束。
...
| 日期 | 版本 | 变更说明 |
|---|---|---|
| 2026-04-14 | v1.0.4 | 增加资源中心规划，明确单位/站点/插件/资源统一治理方向 |
...
- 为保障交付和排障简单性，客户端采用“两层配置”：
  ...
  - `%LOCALAPPDATA%/DibClient/appsettings.json`：用户状态配置
- 该文件仅限本机使用，严禁提交到代码仓库。
- 生效配置加载顺序：`appsettings.json` -> 用户 `appsettings.json` -> 环境变量。
- 主程序配置仅承载宿主部署控制和本机状态，不再承载插件业务资源连接。
- 详细规则见：
