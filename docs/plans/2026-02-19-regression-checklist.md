...
| 项目 | 预期 | 状态 |
|---|---|---|
| 托盘图标显示 | 右下角任务栏或 `^` 区域可见图标 | 通过 |
| 托盘点击切换窗口 | 点击托盘图标可显示/隐藏窗口 | 通过 |
| 导航高亮 | 左侧当前模块高亮（含设置） | 通过 |
| Tab 行为 | 可打开/切换/关闭标签页，首页保底存在 | 通过 |
| 首页快捷入口 | “打开待办/打开设置”可正确跳转 | 通过 |
| Todo 空状态 | 空列表显示空状态；筛选为空可“清空筛选” | 通过 |
| Todo 完成态 | 已完成任务标题划线、内容弱化 | 通过 |
| 设置保存 | 保存后重启应用，设置可读取 | 通过 |
...
- 手动 GUI 回归已完成（2026-02-19，Windows 桌面环境）。
