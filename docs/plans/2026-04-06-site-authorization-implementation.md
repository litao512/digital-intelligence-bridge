# Site Authorization Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 为每个 DIB 安装实例建立站点身份，支持按站点分组批量授权插件，并在发布中心展示站点统计分析。

**Architecture:** 在 `dib_release` schema 中新增站点、分组、授权与心跳表；DIB 客户端在启动或检查更新时上报站点信息；发布中心按 `site_id` 和 `channel` 生成裁剪后的插件 manifest，并提供站点管理与统计页面。

**Tech Stack:** Supabase Postgres、Supabase Auth、Vue 3、TypeScript、Vite、.NET 10、Avalonia、xUnit

---

### Task 1: 扩展数据库 schema 支撑站点与授权
...
**Step 1: Write the failing SQL validation extension**
- 在 `08_validate_release_center_schema.sql` 先增加对新表、新索引、新视图的断言。
- 预期当前执行会失败，因为对象尚不存在。

**Step 2: Create minimal schema objects**
...
Run: 对 `08_validate_release_center_schema.sql` 进行验证执行
Expected: 全部断言通过
...
Run: `npm test -- siteAuthorizationService`
...
**Step 2: Run tests to verify failure**
Run: `dotnet test ... --filter "SiteId|SiteHeartbeat"`
Expected: FAIL

**Step 3: Implement minimal config and service changes**
- 在配置中加入：
...
**Step 4: Run tests to verify pass**
Run: `dotnet test ... --filter "SiteId|SiteHeartbeat|ReleaseCenterServiceTests|SettingsViewModelReleaseCenterTests"`
Expected: PASS
...
**Step 2: Run tests to verify failure**
Run: `npm test` and `dotnet test ... --filter "ReleaseCenterServiceTests|manifestService"`
Expected: FAIL
...
**Step 4: Run tests to verify pass**
Run: `npm test` and `dotnet test ... --filter "ReleaseCenterServiceTests"`
Expected: PASS
...
**Step 3: Implement page and aggregation**
- 在仓储或服务层增加站点统计查询。
- 页面展示：
  - 总览卡片
  - 分组分布
