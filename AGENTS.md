...
## 文档语言规范
- `docs/**/*.md` 默认使用中文撰写。
- `docs/plans/archive/**` 仅作为历史参考，不得直接作为当前执行计划；执行任务时优先使用 `docs/plans/README.md` 中标注的“当前有效文档”。
- 例外项（不翻译）：代码、命令、配置键、路径、协议字段名、环境变量名。
- 术语首次出现使用“中文（English）”格式，后续优先仅使用中文；行业固定缩写（如 PRD、API、CI）可保留。
- 禁止在中文文档主体中出现整段英文句子（引用原文除外，需使用引用块并标注来源）。
- 禁止同级标题出现“中英文混排但非翻译关系”的写法。
- 提交前应运行 `scripts/check-doc-lang.ps1`（可通过 `scripts/install-git-hooks.ps1` 安装 pre-commit 自动检查）。
