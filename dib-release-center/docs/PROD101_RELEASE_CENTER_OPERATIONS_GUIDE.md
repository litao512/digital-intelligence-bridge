...
18. `17_create_resource_center_runtime_tables.sql`
19. `18_seed_resource_center_runtime_baseline.sql`
20. `19_create_get_site_authorized_resources_rpc.sql`
21. `20_create_discover_site_resources_rpc.sql`
22. `21_create_apply_site_resource_rpc.sql`
...
3. 调用 `dib_release.get_site_authorized_resources(...)` 获取已授权资源
4. 调用 `dib_release.discover_site_resources(...)` 获取可申请资源、已授权资源和待处理申请
...
- 为 `patient-registration` 下发 `usage_key = registration-db` 的 PostgreSQL 资源。
...
### 6.2.2 资源中心运行时 RPC 检查

```bash
docker exec -i supabase-db psql -U postgres -d postgres -c "select to_regprocedure('dib_release.get_site_authorized_resources(text,text,text,jsonb)');"
docker exec -i supabase-db psql -U postgres -d postgres -c "select to_regprocedure('dib_release.discover_site_resources(text,text,text,jsonb)');"
docker exec -i supabase-db psql -U postgres -d postgres -c "select to_regprocedure('dib_release.apply_site_resource(text,text,text,text,text,text)');"
```

预期三个 `regprocedure` 都非空。

### 6.2.3 资源中心表检查

```bash
docker exec -i supabase-db psql -U postgres -d postgres -c "select table_name from information_schema.tables where table_schema = 'dib_release' and table_name like 'resource_%' order by table_name;"
```
...
如需先验证客户端正式资源链路，可执行以下思路：

1. 确认目标站点已通过 `register_site_heartbeat` 注册
2. 查询该站点 `sites.id`
3. 查询示例资源 `patient-registration-postgres-sample`
4. 插入一条绑定：
...
join dib_release.resources r on r.resource_code = 'patient-registration-postgres-sample'
where s.site_id = '<目标站点 SiteId>'
on conflict (site_row_id, plugin_code, usage_key) do update
...
1. 不再出现资源中心 `404`
2. `%LocalAppData%\\DibClient\\resource-cache\\authorized-resources.json` 已生成
3. 授权缓存中包含 `patient-registration`
4. `PatientRegistration` 插件可通过正式资源中心链路读取 `registration-db`
