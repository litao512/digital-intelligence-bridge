...
public class SettingsViewModelRollbackTests
{
    [Fact]
    public async Task RestoreLatestPluginBackupCommand_ShouldPopulateRollbackSummary_WhenServiceReturnsSuccess()
    {
        var vm = CreateVm(new StubReleaseCenterService());

        vm.RestoreLatestPluginBackupCommand.Execute();
        await WaitForRollbackAsync(vm);

        Assert.Equal("已恢复最近一次插件备份", vm.PluginRollbackSummary);
        Assert.Equal("已有插件回滚就绪，重启 DIB 后生效。", vm.RestartRequiredNotice);
        Assert.NotNull(vm.LastPluginRollbackAt);
    }

    private static async Task WaitForRollbackAsync(SettingsViewModel vm)
    {
        for (var i = 0; i < 50; i++)
        {
            if (!vm.IsPluginRollbackRunning)
            {
                return;
            }
