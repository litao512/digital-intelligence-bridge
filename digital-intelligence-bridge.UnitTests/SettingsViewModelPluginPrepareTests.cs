...
public class SettingsViewModelPluginPrepareTests
{
    [Fact]
    public async Task PreparePluginPackagesCommand_ShouldPopulatePrepareSummary_WhenServiceReturnsSuccess()
    {
        var vm = CreateVm(new StubReleaseCenterService());

        vm.PreparePluginPackagesCommand.Execute();
        await WaitForPrepareAsync(vm);

        Assert.Equal("已生成 1 个预安装目录", vm.PluginPrepareSummary);
        Assert.NotNull(vm.LastPluginPrepareAt);
    }

    private static async Task WaitForPrepareAsync(SettingsViewModel vm)
    {
        for (var i = 0; i < 50; i++)
        {
            if (!vm.IsPluginPrepareRunning)
            {
                return;
            }
