...
public class MedicalDrugImportPluginExcelTests
{
    [Fact]
    ...
    public async Task ReadRowsAsync_ShouldPopulateBusinessKeyAndMappedFields_WhenWorkbookMatchesTemplate()
    {
        ...
        try
        {
            ...
            var rows = new List<DrugImportRow>();
    ...
    [Fact]
    public async Task ValidateAsync_ShouldUpdatePreviewSummary_WhenViewModelRunsValidation()
    {
        var filePath = CreateWorkbook();
        ...
        {
            var service = new DrugExcelImportService();
            var viewModel = new DrugImportPluginViewModel(service);
            viewModel.SelectedFilePath = filePath;

            await viewModel.ValidateAsync();

            Assert.True(viewModel.IsPreviewValid);
            Assert.Contains("工作表完整", viewModel.PreviewSummary);
            Assert.Equal(2, viewModel.SheetRowCounts["总表（270419）"]);
        }
        finally
        {
            File.Delete(filePath);
    ...
    [Fact]
    public async Task ValidateAsync_ShouldSupportSharedStringsWorkbook_WhenHeadersUseSharedStrings()
    {
        var filePath = CreateSharedStringWorkbook();
        ...
        try
        {
            ...
            Assert.True(preview.IsValid);
            Assert.Equal(1, preview.SheetRowCounts["总表（270419）"]);
        }
        finally
        {
            File.Delete(filePath);
    ...
    private static string CreateWorkbook(bool includeRelationSheet = true)
    {
    ...
    private static string CreateSharedStringWorkbook()
    {
        var filePath = Path.Combine(Path.GetTempPath(), $"plugin-drug-import-shared-{Guid.NewGuid():N}.xlsx");
        using var archive = ZipFile.Open(filePath, ZipArchiveMode.Create);
        ...
        WriteEntry(archive, "xl/workbook.xml", CreateWorkbookXml(true));
        WriteEntry(archive, "xl/_rels/workbook.xml.rels", CreateWorkbookRelsXml(true));
        WriteEntry(archive, "xl/sharedStrings.xml", CreateSharedStringsXml());
        WriteEntry(archive, "xl/worksheets/sheet1.xml", CreateSharedStringSheetXml(DrugExcelTemplate.Default.Sheets[0].Columns, "A001", "药品甲"));
        WriteEntry(archive, "xl/worksheets/sheet2.xml", CreateSharedStringHeaderOnlySheetXml(DrugExcelTemplate.Default.Sheets[1].Columns));
        WriteEntry(archive, "xl/worksheets/sheet3.xml", CreateSharedStringHeaderOnlySheetXml(DrugExcelTemplate.Default.Sheets[2].Columns));
        WriteEntry(archive, "xl/worksheets/sheet4.xml", CreateSharedStringHeaderOnlySheetXml(DrugExcelTemplate.Default.Sheets[3].Columns));
    ...
    private static string CreateSharedStringsXml()
    {
        var strings = new List<string>();
        strings.AddRange(DrugExcelTemplate.Default.Sheets[0].Columns);
        strings.AddRange(DrugExcelTemplate.Default.Sheets[1].Columns);
        strings.AddRange(DrugExcelTemplate.Default.Sheets[2].Columns);
        strings.AddRange(DrugExcelTemplate.Default.Sheets[3].Columns);
        strings.Add("A001");
        strings.Add("药品甲");

        var items = string.Join(string.Empty, strings.Select(value => $"<si><t>{System.Security.SecurityElement.Escape(value)}</t></si>"));
        return $$"""
        <?xml version="1.0" encoding="UTF-8" standalone="yes"?>
    ...
    private static string CreateSharedStringSheetXml(IReadOnlyList<string> headers, string drugCode, string name)
    {
        var headerCells = string.Join(
            string.Empty,
            headers.Select((_, index) => $"""<c r="{ColumnName(index + 1)}1" t="s"><v>{index}</v></c>"""));

        var firstDataIndex = headers.Count * 4;
        var nameColumnIndex = headers.ToList().IndexOf("注册名称") + 1;
        var row2Cells = $"""
            <c r="A2" t="s"><v>{firstDataIndex}</v></c>
            <c r="{ColumnName(nameColumnIndex)}2" t="s"><v>{firstDataIndex + 1}</v></c>
            """;
    ...
    private static string CreateSharedStringHeaderOnlySheetXml(IReadOnlyList<string> headers)
    {
        var offset = 0;
        if (headers.SequenceEqual(DrugExcelTemplate.Default.Sheets[1].Columns))
        {
            offset = DrugExcelTemplate.Default.Sheets[0].Columns.Count;
        }
        else if (headers.SequenceEqual(DrugExcelTemplate.Default.Sheets[2].Columns))
        {
            offset = DrugExcelTemplate.Default.Sheets[0].Columns.Count + DrugExcelTemplate.Default.Sheets[1].Columns.Count;
        }
        else if (headers.SequenceEqual(DrugExcelTemplate.Default.Sheets[3].Columns))
        {
            offset = DrugExcelTemplate.Default.Sheets[0].Columns.Count + DrugExcelTemplate.Default.Sheets[1].Columns.Count + DrugExcelTemplate.Default.Sheets[2].Columns.Count;
        }

        var headerCells = string.Join(
            string.Empty,
            headers.Select((_, index) => $"""<c r="{ColumnName(index + 1)}1" t="s"><v>{offset + index}</v></c>"""));
