...
public class SettingsViewModelRestartNoticeTests
{
    [Fact]
    public async Task DownloadPluginPackagesCommand_ShouldSetRestartNotice_WhenPackagesDownloaded()
    {
        var vm = CreateVm(new StubReleaseCenterService());

        vm.DownloadPluginPackagesCommand.Execute();
        await WaitForAsync(() => !vm.IsPluginDownloadRunning);

        Assert.Equal("已有插件更新就绪，重启 DIB 后生效。", vm.RestartRequiredNotice);
    }

    [Fact]
    public async Task PreparePluginPackagesCommand_ShouldSetRestartNotice_WhenPackagesPrepared()
    {
        var vm = CreateVm(new StubReleaseCenterService());

        vm.PreparePluginPackagesCommand.Execute();
        await WaitForAsync(() => !vm.IsPluginPrepareRunning);
